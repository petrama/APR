...
/**
 * Apstraktan razred koji predstavlja rad sa matricom. Razred nudi standardne
 * operacije s matricama kao sto su zbrajanje, oduzimanje,matricno mnozenje,
 * računanje determinante i inverza. Osim operacija nudi transponiranje te
 * dobivanje algebarskih komplementa.
 * 
 * @author Petra Marče
 *
 */
public abstract class AbstractMatrix implements IMatrix {
	...
	/**
	 * Metoda omogucava stvaranje transponirane matrice u odnosu na originalnu
	 * matricu. Ako je parametar liveView postavljen na true, objekt koji se
	 * vrati mora biti "zivi" pogled na originalnu matricu. U suprotnom stvara
	 * se nova matrica koja cuva vlastitu kopiju podataka.
	 * 
	 * @param liveView
	 *            zeli li se dobiti zivi pogled
	 ...
	 * 
	 * @param other
	 *            matrica s kojom treba pomnoziti trenutnu
	 * @return referenca na novu matricu
	 */
	@Override
	 ...
	 * na trenutnu matricu. Ako je parametar liveView postavljen na true, metoda
	 * mora vratiti zivi pogled na izvornu matricu; to znaci da, primjerice, ako
	 * u ovom pogledu izmjenimo neki element, promjena ce se vidjeti i u
	 ...
	 * kofaktora kako je to opisano u knjizi. Ovdje nije naglasak na efikasnosti
	 * implementacije. U slucaju da matrica nije invertibilna, ocekuje se da ce
	 * biti izazvana {@link UnsupportedOperationException}.
	 * 
	 * @return referencu na novu matricu koja je inverz trenutne
	 */
	@Override
	public IMatrix nInvert() {
		return nInvertUsingLUP();
//		if (this.getColsCount() != this.getRowsCount()) {
//			throw new IncompatibleOperandException(
...
//		return this.cofactor().scalarMultiply(1 / detA).nTranspose(true);
	...
	public  IMatrix nInvertUsingLUP() {
		if (this.getColsCount() != this.getRowsCount()) {
			throw new IncompatibleOperandException(
					"Only square matrix is possible to invert!");
		...
		int n = getRowsCount();
		IMatrix lu=this.copy();
		IMatrix perm = LUP.getLUMatrix(lu, true);
		IMatrix inverz = this.copy().makeIdentity();
		...
		for (int i = 0; i < n; i++) {
			...
			IMatrix b=new Vector(vecData).toColumnMatrix(true);
			IMatrix y=LUP.unaprijed(lu, perm.nMultiply(b));
			IMatrix x=LUP.unatrag(lu, y);
			IVector xx=x.toVector(true);
			for(int j=0;j<n;j++){
				inverz.set(j, i, xx.get(j));
			}
		}
		return inverz;
	...
	/**
	 * Metoda koja vraca matricu istih dimenzija kao sto je zadana. Elementi
	 * nove matrice na su algebarski komplementi pocetne matrice,tj determinante
	 * matrice koja se dobije izbacivanjem i-tog retka i j-tog stupca.
	 * 
	...
	/**
	 * Metoda koja vraća stringovnu reprezentaciju matrice. Elementi matrice
	 * ispisuju se na 3 decimale.
	 */
	@Override
	...
	/**
	 * Metoda vraća stringovnu reprezentaciju matrice. Elementi matrice su u
	 * prozivoljnoj preciznosti.
	 * 
	 * @param precision
	 *            zadana preciznost u kojoj se ispisuju elementi.
	 ...
	 * matrice pomnoženima s zadanom vrijednosti.
	 * 
	 * @param value
